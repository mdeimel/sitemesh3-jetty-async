...
public abstract class ContentBufferingFilter implements Filter {
    ...
    /**
     * Apply next filter/servlet to the buffer.
     */
            ...
            request.getAsyncContext().addListener(new AsyncListener() {
                ...
                public void onComplete(AsyncEvent asyncEvent) throws IOException {
                    try {
                        ...
                        throw new RuntimeException("Could not execute async request.", e);
                    }
                }

                @Override
                public void onTimeout(AsyncEvent asyncEvent) throws IOException {
                    throw new RuntimeException("Timeout during async request handling.", asyncEvent.getThrowable());
                }

                @Override
                public void onError(AsyncEvent asyncEvent) throws IOException {
                    throw new RuntimeException("Error during async request handling.", asyncEvent.getThrowable());
                }
    ...
    /**
     * Post process the response and send to the real response.
